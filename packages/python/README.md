...
## Exported Node.js event type

When using the companion Node.js package, import its canonical class (with a lowercase `t`) and exported event type directly:
...
| `on_warning` | `Callable[[LLMtrackWarning], None] \| None` | logs one `[llmtrack]` warning | Receives pricing warnings, once per distinct warning per process. |
...
A short-lived process can exit before the daemon thread finishes; use `await track_sync()` in scripts and jobs that must confirm delivery, and inspect its returned API dictionary.
