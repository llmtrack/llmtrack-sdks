...
## Use the exported event type

Import the canonical `LLMtrack` class (with a lowercase `t`) and the exported `TrackOptions` type directly from the package:
...
You can then pass `event` to `tracker.track(event)` or `await tracker.trackSync(event)`.
...
| `onWarning` | `(warning: LLMtrackWarning) => void` | logs one `[llmtrack]` warning | Receives unknown-pricing warnings. Each distinct warning is emitted once per process. |
...
Confirm the constructor/event environment and that the process lives long enough for fire-and-forget delivery; use `await trackSync()` in short-lived scripts to inspect the result and confirm delivery.
